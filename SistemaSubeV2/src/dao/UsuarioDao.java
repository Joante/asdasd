...
public class UsuarioDao {
	...
	public Usuario traerUsuario2(long dni) throws HibernateException {
		Usuario objeto = null;
		try {
			iniciaOperacion();
	...
	public Usuario accesoLogin(String pass, long dni) throws HibernateException {
		Usuario objeto = null;
		try {
			iniciaOperacion();
			objeto = (Usuario) session.createQuery("from Usuario c where c.password='"+pass+"' and c.dni="+dni).uniqueResult();
		}
		finally {
			session.close();
		}
